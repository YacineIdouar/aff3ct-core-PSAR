...
int main(int argc, char** argv)
{
   ...
   // Création de 20 modules relay_io pour le passage de la donnée en mode IO
	...
	// sockets binding
	(*rlys_io[0])[module::rly_io::sck::relay_io::inout] = source[module::src::sck::generate::out_data]; // Bind du premier relay avec la source
	...
	sink[module::snk::sck::send_count::in_data] = (*rlys_io[rlys_io.size() -1])[module::rly_io::sck::relay_io::inout];
		...
		pipeline_chain.reset(new runtime::Pipeline(
		                     ...
		                     { // pipeline stage 0
		                       ...
		                       { { &(*rlys_io[             0])[module::rly_io::tsk::relay_io] },   // first tasks of stage 1
		                         { &(*rlys_io[rlys_io.size() -1])[module::rly_io::tsk::relay_io] } }, // last  tasks of stage 1
		                       // pipeline stage 2
	...
	(*rlys_io[0])[module::rly_io::sck::relay_io::inout].unbind(source[module::src::sck::generate::out_data]); // Unbind de la source et du premier module de l'étage 1
	for (size_t s = 0; s < rlys_io.size() -1; s++)
